...
```bash
# Build
xcodebuild -scheme FindYourWall -destination 'platform=iOS Simulator,name=iPhone 17' build

# Run all tests
xcodebuild -scheme FindYourWall -destination 'platform=iOS Simulator,name=iPhone 17' test

# Run a specific test suite
xcodebuild -scheme FindYourWall -destination 'platform=iOS Simulator,name=iPhone 17' test -only-testing:FindYourWallTests/MapViewModelTests
```

## Architecture

- **SwiftUI + SwiftData** universal app (iPhone and iPad).
